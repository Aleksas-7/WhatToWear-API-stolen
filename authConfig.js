...
passport.use(
    new LocalStrategy(async (username, password, done) => {
      try {
        const res = await pool.query('SELECT * FROM users WHERE username = $1 AND provider = $2', [username, 'local']);
        const user = res.rows[0];
        if (!user) return done(null, false, { message: 'Incorrect username or password' });
  
        const isMatch = await bcrypt.compare(password, user.password);
        return isMatch ? done(null, user) : done(null, false, { message: 'Incorrect username or password' });
      } catch (error) {
        return done(error);
      }
    })
...
passport.use(
  new GoogleStrategy(
    ...
    async (accessToken, refreshToken, profile, done) => {
      try {
        const user = await findOrCreateUser('google', profile.id, profile);
        done(null, user);
      } catch (error) {
        done(error);
      }
...
const registerUser = async (username, email, password) => {
    ...
    const query = 'INSERT INTO users(username, email, password,provider) VALUES($1, $2, $3,$4)';
...
//     async (accessToken, refreshToken, profile, done) => {
//       try {
//         const user = await findOrCreateUser('facebook', profile.id, profile);
//         done(null, user);
//       } catch (error) {
//         done(error);
//       }
